# Boiler Plate - please ensure you have all these packages installed
    ...
    try:
        ...
        for exp_date, chain in options_chains.items():
            ...
            if straddle is None:
                ...
                call_last = calls.loc[call_idx, 'lastPrice']
                put_last = puts.loc[put_idx, 'lastPrice']
                
                call_mid = None
                put_mid = None
                
                # Calculate call mid
                if call_bid is not None and call_ask is not None and call_bid > 0 and call_ask > 0:
                    call_mid = (call_bid + call_ask) / 2.0
                elif call_last is not None and call_last > 0:
                    call_mid = call_last
                
                # Calculate put mid
                if put_bid is not None and put_ask is not None and put_bid > 0 and put_ask > 0:
                    put_mid = (put_bid + put_ask) / 2.0
                elif put_last is not None and put_last > 0:
                    put_mid = put_last
                
                # Calculate straddle if we have both prices
        ...
        # Calculate Metrics
        # Metric One: Term Structure
        term_spline = build_term_structure(dtes, ivs)
        ts_slope_0_45 = (term_spline(45) - term_spline(dtes[0])) / (45 - dtes[0])
        
        # Metric Two: Relative IV/RV
        price_history = stock.history(period='3mo')
        rv30 = yang_zhang(price_history)
        iv30 = term_spline(30)
        iv30_rv30 = iv30 / rv30

        # Metric Three: Trading Volume
